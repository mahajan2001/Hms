...
class Login extends MyController
{
    ...
    public function login()
    {
        ...
        if (!empty($adminemail) && !empty($adminpassword)) {
            $model = new Sitefunction();
            $admin_find = $model->get_single_row(TBL_ADMIN, '*', array('admin_email' => strtolower($adminemail), 'status' => 1));
            if (!empty($admin_find)) {
                $storepass = $admin_find['admin_password'];
                if ($this->verify_password($adminpassword, $storepass)) {
                    $this->session->set('admin_id', strval($admin_find["id"]));
                    $this->session->set('admin_name', $admin_find["admin_name"]);
                    $dataArray = array(
                        'adminId' =>  $admin_find["id"],
                        'email' => $adminemail,
                        'contact' => $admin_find["admin_contact"],
                    );
                    $key = $this->getKey();
